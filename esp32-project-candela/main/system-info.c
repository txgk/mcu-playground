...
static volatile bool can_driver_is_started = false;
...
get_system_info_string(const char *value, char *answer_buf_ptr, int *answer_len_ptr)
{
	...
	*answer_len_ptr = snprintf(
		...
		__TIME__,
		esp_get_idf_version(),
		can_driver_is_started ? "active" : "inactive",
...
void
tell_them_that_can_driver_is_started(void)
{
	can_driver_is_started = true;
}

bool
check_if_can_driver_is_started(void)
{
	return can_driver_is_started;
}
