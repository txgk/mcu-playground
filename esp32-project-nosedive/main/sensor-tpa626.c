...
tpa626_task(void *dummy)
{
	...
	while (true) {
		...
		if (xSemaphoreTake(system_mutexes[MUX_I2C_DRIVER], portMAX_DELAY) == pdTRUE) {
			...
			current = (double)tpa626_read_two_bytes_from_register(TPA626_REGISTER_CURRENT) * 0.001;
			power = (double)tpa626_read_two_bytes_from_register(TPA626_REGISTER_POWER) * 25 * 0.001;
			xSemaphoreGive(system_mutexes[MUX_I2C_DRIVER]);
		}
		tpa626_text_len = snprintf(
			tpa626_text_buf,
			TPA626_MESSAGE_SIZE,
			"TPA626@%lld=%.2lf,%.2lf,%.2lf,%.2lf\n",
			ms,
			shunt_voltage,
			bus_voltage,
			current,
